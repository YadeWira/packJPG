...
// Parsed PJG header codes. Filled by pjg_read_header_codes(), shared by the
// decoder and the lister so both accept exactly the same set of files.
struct pjg_header_info {
	unsigned char version;        // version byte (>= 0x14) that ended the header
	bool          parallel_fmt;   // 0x01 seen: -sfth parallel component streams
	bool          jpegls;         // 0x03 seen: pixels stored as JXL + template
	bool          v40b_features;  // 0x02 seen: diagonal DC ctx and later features
	bool          has_settings;   // 0x00 seen: nois_trs/segm_cnt came from the file
	unsigned char nois_trs[ 4 ];
	unsigned char segm_cnt[ 4 ];
};
INTERN bool pjg_read_header_codes( pjg_header_info* info );
...
[[maybe_unused]] INTERN const char*  versiondate  = "08/25/2026";
...
/* -----------------------------------------------
	Shared PJG header-code reader.

	Both the decoder (unpack_pjg) and the lister (list_pjg) go through this,
	deliberately: the lister used to have its own, looser walk over the
	same bytes — it ignored every read() return, so a truncated .pjg produced
	an invented version number reported as a valid file ("1 ok, 0 errors")
	while the decoder rejected the same bytes. Two validations that must agree
	are two validations that eventually will not; the fix is one validation
	with two callers, not a second one kept in sync by hand.

	Reads header codes until the version byte, filling *info. On any failure
	sets errormessage/errorlevel and returns false, so a caller can surface
	the decoder's own diagnostic verbatim.
	----------------------------------------------- */
INTERN bool pjg_read_header_codes( pjg_header_info* info )
{
	unsigned char hcode;
	...
	while ( true ) {
		...
		if ( hcode == 0x00 ) {
			// compression settings stored in the file
			if ( str_in->read( info->nois_trs, 4 ) != 4 ||
			     str_in->read( info->segm_cnt, 4 ) != 4 ) {
				snprintf( errormessage, MSG_SIZE, "unexpected end of file in PJG settings block" );
				errorlevel = 2;
				return false;
			}
...
INTERN bool unpack_pjg( void )
{
	...
	// check header codes
	pjg_header_info hdr;
	if ( !pjg_read_header_codes( &hdr ) ) return false;
	hcode              = hdr.version;
	parallel_fmt       = hdr.parallel_fmt;
	decoded_from_sfth  = hdr.parallel_fmt;
	jpg_jpegls         = hdr.jpegls;
	if ( hdr.jpegls ) jpg_was_jpegls_out = true;
	v40b_features      = hdr.v40b_features;
	format_version_read = hdr.version;
	if ( hdr.has_settings ) {
		memcpy( nois_trs, hdr.nois_trs, 4 );
		memcpy( segm_cnt, hdr.segm_cnt, 4 );
		auto_set = false;
	}
...
INTERN bool list_pjg( void )
{
	...
	// Walk the header with the DECODER's reader, not a private copy of it:
	// whatever `x` would refuse, `list` refuses too, with the same message.
	// A truncated .pjg used to be listed here as a valid file with an invented
	// version number, because this function had its own walk that ignored
	// every read() return.
	str_in->rewind();
	unsigned char magic[2];
	if ( str_in->read( magic, 2 ) != 2 ||
	     magic[0] != pjg_magic[0] || magic[1] != pjg_magic[1] ) {
		snprintf( errormessage, MSG_SIZE, "not a PJG file (bad or truncated magic)" );
		errorlevel = 2;
		return false;
	}
	...
	// JPEG-LS payloads are the one part of a .pjg whose length is declared in
	// the file, so it is the one part `list` can check without decoding: the
	// declared recipe and JXL sizes must fit in what is left. Everything else
	// after the header is an arithmetic stream with no length to compare
	// against, which is why the note below exists instead of a deeper check.
	if ( hdr.jpegls ) {
		long remaining = pjg_size - (long) str_in->num_bytes_read();
		uint8_t le[4];
		for ( int part = 0; part < 2; part++ ) {
			if ( str_in->read( le, 4 ) != 4 ) {
				snprintf( errormessage, MSG_SIZE, "JPEG-LS: truncated %s size",
					( part == 0 ) ? "recipe" : "JXL" );
				errorlevel = 2;
				return false;
			}
			remaining -= 4;
			uint32_t sz = le[0] | ((uint32_t)le[1]<<8) | ((uint32_t)le[2]<<16) | ((uint32_t)le[3]<<24);
			if ( (long) sz > remaining ) {
				snprintf( errormessage, MSG_SIZE,
					"JPEG-LS: declared %s size %u B exceeds the %ld B left in the file",
					( part == 0 ) ? "recipe" : "JXL", sz, remaining );
				errorlevel = 2;
				return false;
			}
			str_in->skip( (int) sz );
	...
	bool is_sfth = hdr.parallel_fmt;
	int ver_major = hdr.version / 10;
	int ver_minor = hdr.version % 10;
	...
	// Say what was checked. `list` reads the header (and, for JPEG-LS, the
	// declared part sizes); it does not decode, so a file that lists cleanly
	// can still fail to decompress. Reporting "ok" without this line is the
	// lister answering a smaller question than the one the user is asking.
