...
// XP build: safe_path returns std::string directly (no filesystem::path).
static std::string safe_path( const char* s )
{
...
#define THREAD_LOCAL  /* XP: no thread_local */ static
...
// JPEG-LS (SOF F7) lossless recompression state.
...
// used to rename the default .jpg output to .jls. Cleared per-file in check_file().
...
// Blowup-ratio bomb guard: reconstructed JPEG must not exceed
// input PJG size * PJG_MAX_BLOWUP_RATIO + PJG_BLOWUP_FLOOR_BYTES. 500x is
// ~10x above the most extreme legitimate case (solid-color sky + metadata
// stripping). Always active — the amplification vector is inherent to
// lossless compression and is present in upstream packJPG too.
#define PJG_MAX_BLOWUP_RATIO  500
#define PJG_BLOWUP_FLOOR_BYTES 1048576  // 1 MB — tiny files produce tiny output
...
/* -----------------------------------------------
	...
	----------------------------------------------- */
static void collect_files_recursive( const std::string& dir,
                                     std::vector<std::pair<std::string,std::string>>& out )
{
    xp_collect_files_recursive_with_root( dir, dir, out );
}
...
{	
	...
	while ( --argc > 0 ) {
		...
		if ( strcmp((*argv), "-p" ) == 0 ) {
			...
			fprintf( msgout, "\nNote: -th is not supported in XP build, ignored.\n" );
		}
			...
			// Default is 256 MB; 0 = unlimited. A malformed .pjg can expand a
			// tiny input into a huge JPEG; with this set, such a decode fails
			// cleanly instead of producing it.
			if ( tmp_val < 0 )    tmp_val = 0;
			if ( tmp_val > 4095 ) tmp_val = 4095; // keep MB*1MB within unsigned int
			if ( tmp_val == 0 )
				pjg_max_output_size = 0;          // explicit "no limit"
			else
			...
			outdir = (*argv) + 3;
			// Create directory if it doesn't exist (XP: use Win32)
			xp_create_directories( outdir ? outdir : "" );
			// failure silently ignored — will surface later when writing files
		}
			...
			*(tmp_flp++) = (char*) "-";
			...
			{
			  bool is_dir = xp_is_directory( *argv );
			  if ( !is_dir || recursive )
				*(tmp_flp++) = *argv;
			}
			#endif
	...
	if ( recursive ) {
		...
		for ( int fi = 0; filelist[ fi ] != NULL; fi++ ) {
			std::string p = safe_path( filelist[ fi ] );
			if ( xp_is_directory( p ) ) {
...
INTERN void process_ui( void )
{
	...
	if ( !pipe_on && filetype == F_PJG && action == A_COMPRESS &&
...
INTERN bool merge_jpeg( void )
{
	...
	if ( jpg_jpegls ) {
		char jls_err[256]; uint8_t* recon = nullptr; size_t recon_sz = 0;
		if ( !jpegls_reconstruct( jpegls_info, jpegls_template, jpegls_template_sz,
		                          jpegls_pixels, jpegls_pixels_sz,
		                          recon, recon_sz, jls_err, sizeof(jls_err) ) ) {
	...
	// Decompression-bomb guard (pre-decode estimate): refuse to
	// reconstruct a JPEG larger than the cap or the blowup-ratio limit.
	// The estimate (SOI+headers+huffman+garbage+EOI) is a lower bound —
	// 0xFF stuffing and restart markers only add to it.
	{
		...
		if ( est > ratio_limit ) {
			snprintf( errormessage, MSG_SIZE,
				"blowup ratio exceeded: reconstructed JPEG would be at least "
				"%lld bytes from %lld-byte input (limit %lld)",
	...
	// Ratio-based bomb guard (exact): same check on the actual output.
	{
		int64_t ratio_limit;
		if ( pjgfilesize > (int64_t)( INT64_MAX / PJG_MAX_BLOWUP_RATIO ) )
			ratio_limit = INT64_MAX;
		...
		if ( (uint64_t) jpgfilesize > (uint64_t) ratio_limit ) {
			snprintf( errormessage, MSG_SIZE,
				"blowup ratio exceeded: reconstructed JPEG is %lld bytes "
				"from %lld-byte input (limit %lld)",
...
INTERN bool pack_pjg( void )
{
	...
	if ( jpg_jpegls ) {
		char jls_err[256]; uint8_t* jxl = nullptr; size_t jxl_sz = 0;
		if ( !jpegls_compress_jxl( jpegls_pixels, jpegls_pixels_sz,
		                            jpegls_info.width, jpegls_info.height,
		                            jpegls_info.bits_per_sample, jpegls_info.component_count,
		...
		uint8_t rle[4] = { (uint8_t)rsz, (uint8_t)(rsz>>8), (uint8_t)(rsz>>16), (uint8_t)(rsz>>24) };
		str_out->write( rle, 4 ); str_out->write( hdrdata, hdrs );

		uint32_t jsz = (uint32_t) hufs;
		uint8_t jle[4] = { (uint8_t)jsz, (uint8_t)(jsz>>8), (uint8_t)(jsz>>16), (uint8_t)(jsz>>24) };
		str_out->write( jle, 4 ); str_out->write( huffdata, hufs );
...
INTERN bool unpack_pjg( void )
{
	...
	if ( jpg_jpegls ) {
		...
		if ( !str_in->read( rle, 4 ) ) {
			snprintf( errormessage, MSG_SIZE, "JPEG-LS: truncated recipe size" ); errorlevel = 2; return false;
		}
		uint32_t rsz = rle[0] | ((uint32_t)rle[1]<<8) | ((uint32_t)rle[2]<<16) | ((uint32_t)rle[3]<<24);
		if ( hdrdata ) free( hdrdata );
		hdrdata = (unsigned char*) malloc( rsz );
		if ( !hdrdata ) { snprintf( errormessage, MSG_SIZE, "JPEG-LS: OOM recipe" ); errorlevel = 2; return false; }
		if ( !str_in->read( hdrdata, rsz ) ) {
			snprintf( errormessage, MSG_SIZE, "JPEG-LS: truncated recipe data" ); errorlevel = 2; return false;
		}
		hdrs = (int) rsz;

		uint8_t jle[4];
		if ( !str_in->read( jle, 4 ) ) {
			snprintf( errormessage, MSG_SIZE, "JPEG-LS: truncated JXL size" ); errorlevel = 2; return false;
		}
		uint32_t jsz = jle[0] | ((uint32_t)jle[1]<<8) | ((uint32_t)jle[2]<<16) | ((uint32_t)jle[3]<<24);
		if ( huffdata ) free( huffdata );
		huffdata = (unsigned char*) malloc( jsz );
		if ( !huffdata ) { snprintf( errormessage, MSG_SIZE, "JPEG-LS: OOM JXL" ); errorlevel = 2; return false; }
		if ( !str_in->read( huffdata, jsz ) ) {
			snprintf( errormessage, MSG_SIZE, "JPEG-LS: truncated JXL data" ); errorlevel = 2; return false;
		}
		hufs = (int) jsz;

		{
			char jls_err[256]; uint32_t w, h; uint8_t bps, comps;
			if ( !jpegls_decompress_jxl( huffdata, hufs, jpegls_pixels, jpegls_pixels_sz,
			                             w, h, bps, comps, jls_err, sizeof(jls_err) ) ) {
				snprintf( errormessage, MSG_SIZE, "JPEG-LS JXL decode: %s", jls_err ); errorlevel = 2; return false;
			}
		}

		{
			...
			if ( !jpegls_deserialize_recipe( hdrdata, hdrs, jpegls_info,
			                                 ...
			                                 jls_err, sizeof(jls_err) ) ) {
				snprintf( errormessage, MSG_SIZE, "JPEG-LS recipe: %s", jls_err ); errorlevel = 2; return false;
			}
		}
		return true;
	}
	...
	if ( parallel_fmt ) {
		...
		for ( cmp = 0; cmp < cmpc; cmp++ ) {
		// Decompression-bomb guard: reject oversized component streams.
		// csizes are raw uint32_t from the PJG — cap each at the output limit.
		{
			unsigned int csiz_cap = pjg_max_output_size > 0 ? pjg_max_output_size : 64u * 1024 * 1024;
			if ( csizes[cmp] > csiz_cap ) {
				snprintf( errormessage, MSG_SIZE,
					"sfth component stream too large: %u bytes (limit %u)",
					csizes[cmp], csiz_cap );
				errorlevel = 2; return false;
			}
		}
...
INTERN inline char* create_filename( const char* base, const char* extension )
{
	...
	if ( outdir != NULL ) {
		...
		if ( fs_mode && filelist_srcroot != NULL
			...
			// Compute parent dir of `base`
			std::string parent_dir;
			if ( sep != NULL ) parent_dir.assign( base, (size_t)(sep - base) );
			else parent_dir = base;
			std::string rel = xp_relative_path( parent_dir, filelist_srcroot[ file_no ] );
			if ( !rel.empty() && rel != "." ) {
				fs_subdir = rel;
			}
		}
	...
	if ( outdir != NULL ) {
		...
		if ( !fs_subdir.empty() ) {
			...
			strcat( filename, "/" );
			xp_create_directories( std::string( filename ) );
		}
